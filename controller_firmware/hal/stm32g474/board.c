/*
 * board.c - GPIO setup for the general purpose pins, SPI and CAN.
 * (PWM pins are set up in pwm.c; analog pins are analog from reset.)
 */
#include "board.h"
...
void board_init(void)
{
    ...
    /* set output levels before switching pins to outputs:
       chip selects idle high, gate driver disabled */
    GPIOB->BSRR = (1u << ENC_CS_PIN) | (1u << (ENABLE_PIN + 16));
    GPIOC->BSRR = 1u << GD_CS_PIN;
    GPIOB->PUPDR &= ~(3u << (ENABLE_PIN * 2));     /* PB4 has a pull-up from reset (JTAG) */
